...
doRecord (At dt (VR name amount)) = do
  ...
  if getValue delta > 0.0
    then do
           inc <- getIncFrom name
           let post = At dt $ PR $ Transaction 'A' "Autogenerated posting" [name :<+ (F delta), Auto $ accName inc]
           doRecord post
    else if getValue delta == 0.0
           then return ()
           else do
                  dec <- getDecTo name
                  let post = At dt $ PR $ Transaction 'A' "Autogenerated posting" [name :<+ (F delta), Auto $ accName dec]
                  doRecord post
